...
export default function ActivatePage({ initialEmail = '', onActivated, onBackToLogin }) {
  ...
  function handleSubmit(e) {
    ...
    setFormError('')
    activate.mutate({ email, password }, { onSuccess: (data) => onActivated(data.user) })
  }

  const loading = activate.isPending
  const error = formError || (activate.isError ? activate.error.message : '')
    ...
    <div className="min-h-screen flex">
      <div className="hidden lg:flex w-1/2 bg-gradient-to-br from-sky-500 to-indigo-700 flex-col justify-between p-12 text-white">
        ...
        <div>
          <h1 className="text-4xl font-bold leading-tight mb-4">
            Welcome to the team.<br />Let's get you set up.
          </h1>
          <p className="text-sky-100 text-sm leading-relaxed max-w-xs">
            Set a password to activate your account and start working with your team.
          </p>
        </div>
      ...
      <div className="flex-1 flex items-center justify-center bg-gray-50 p-6">
        <div className="w-full max-w-sm">
          ...
          <p className="text-sm text-gray-500 mt-1 mb-8">Set a password to finish joining your team's workspace.</p>
